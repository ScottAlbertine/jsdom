...
module.exports = function createUtils(window) {
  ...
  function validCORSPreflightHeaders(xhr, response, flag, properties, origin) {
    if (!validCORSHeaders) {
      return false;
    }
  ...
  function createClient(xhr) {
    ...
    if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || properties.uploadListener)) {
      ...
      preflightClient.on("response", resp => {
        ...
        if (!validCORSPreflightHeaders(xhr, resp, flag, properties, properties.origin)) {
          setResponseToNetworkError(xhr);
          return;
        }
  ...
  return {
    headerListSeparatorRegexp, simpleHeaders, preflightHeaders, getRequestHeader, updateRequestHeader, createClient,
    dispatchError, requestErrorSteps, setResponseToNetworkError, validCORSHeaders
  };
};
